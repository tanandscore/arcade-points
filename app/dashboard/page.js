...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: scores }, games, categories, continuePlaying, trending, recentActivity, streakDays, dailyChallenges, { data: siteSettings }] = await Promise.all([
    ...
    supabase.from("site_settings").select("launch_countdown_at, launch_countdown_label").eq("id", 1).maybeSingle(),
  ]);
  ...
  // Live-tournament flash on game cards — findActiveTournamentForGame
  // already only ever returns a status "active" tournament (one that
  // has genuinely started, not one still upcoming), so this can never
  // show a countdown before it makes sense to.
  const tournaments = await getTournaments();
  const tournamentBySlug = {};
  for (const g of games) {
    const t = findActiveTournamentForGame(tournaments, g.slug);
    if (t) tournamentBySlug[g.slug] = { name: t.name, endsAt: t.ends_at, locked: t.require_tournament_entry === true };
  }
  ...
  return (
    <div className="min-h-screen bg-bgDeep text-textLight pb-20 sm:pb-0">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        {countdownActive && (
          <div className="mb-8 rounded-xl border border-accentCyan/40 bg-bgPanel px-6 py-5 text-center">
            ...
            <p className="font-mono text-[10px] text-textDim mt-3">You can browse everything now — gameplay unlocks the moment this hits zero.</p>
          </div>
        )}
              ...
              {continuePlayingGames.map((game) => (
                <div key={game.slug} className="w-64 shrink-0">
                  <GameCard
                    ...
                    activeTournament={tournamentBySlug[game.slug]}
