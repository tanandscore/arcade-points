...
export default async function LandingPage() {
  ...
  const countdownActive = siteSettings?.launch_countdown_at && new Date(siteSettings.launch_countdown_at) > new Date();

  // Same ordering as the dashboard, so what a visitor sees here
  // matches exactly what they see after signing up — Arcade first
  // (the site's main highlight), Legend Pass (flagship) next, then
  // Power Pass, then everything else in its normal order.
    ...
    <div className="min-h-screen bg-bgDeep text-textLight">
      ...
      <MarqueeBar
        rightSlot={
          <div className="flex items-center gap-2">
            <Link href="/leaderboard" className="font-mono text-[10px] px-3 py-1.5 rounded-md border border-lineColor text-textDim hover:bg-bgPanel3 transition-colors hidden sm:inline-block">
              🏆 Leaderboard
            </Link>
      ...
      >
        <div className="max-w-4xl mx-auto">
          ...
          {countdownActive && (
            <div className="mb-8 inline-block rounded-xl border border-accentCyan/40 bg-bgPanel px-6 py-4">
              <p className="font-pixel text-[10px] text-accentCyan mb-3">{siteSettings.launch_countdown_label || "LAUNCHING SOON"}</p>
          ...
          <p className="text-textDim text-base sm:text-lg max-w-xl mx-auto mb-10 leading-relaxed">
            {freeCount} games are completely free to play, worldwide, right now. Perfect for a quick break —
            real leaderboards, one account, no downloads.
          </p>
          <p className="font-pixel text-[10px] text-accentCyan ap-blink mb-1.5">▸ PRESS START ▸</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/signup"
              className="font-pixel text-[11px] px-8 py-4 rounded-md bg-accentCyan text-bgDeep w-full sm:w-auto text-center"
            >
              PLAY FREE ▸
            </Link>
            <Link
          ...
          <p className="font-mono text-[11px] text-textDim mt-6">
            No credit card needed, ever, for {freeCount} of the {games.length} games.
          </p>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-20">
        ...
        <FlagshipBanner games={games} bestByGame={{}} />
      ...
      <footer className="border-t border-lineColor py-6 text-center text-[11px] text-textDim font-mono">
        <p>Tap & Score · tapandscore.com</p>
        <p className="mt-1">
          © {new Date().getFullYear()} Tap & Score. All games, code, and artwork on this site are original works
          owned by Tap & Score — copying or redistribution is not permitted without written permission.
        </p>
