...
// Paddle is now the site's only payment provider — PayU and Razorpay
// were both removed after being rejected as gateways for this
// business. Paddle's checkout is a real overlay opened directly from
// the browser via Paddle.js, not a server-created order the way
// Razorpay/PayU worked — so this component initializes its own
// Paddle.js instance and opens the checkout itself, rather than
// calling a backend "create order" route first.
//
// Offers both monthly and annual as two separate, clearly-labeled
// buttons rather than a toggle — the removed PayU flow only ever
...
export default function PaddleSubscribeButton({
  ...
  useEffect(() => {
    ...
    // pwCustomer links this session to Paddle Retain for a signed-in
    // customer — only included when we already have their real
    // Paddle customer ID (ctm_...) on file from a prior subscription.
    // Retain itself only loads on live accounts (verified against
    // Paddle's current docs), so this is harmless to include in
    // sandbox too — it's simply ignored there. Omitted entirely for a
    // user with no Paddle ID yet, rather than guessing at Paddle's
    // "empty object" case, which their own docs describe as meaning
    // "no logged-in customer at all", not "logged in but no Paddle ID yet".
    const initOptions = { environment, token };
    if (paddleCustomerId) initOptions.pwCustomer = { id: paddleCustomerId };
    initializePaddle(initOptions)
      .then((instance) => {
        if (instance) {
          setPaddle(instance);
          setInitStatus("ready");
