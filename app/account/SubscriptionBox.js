...
export default function SubscriptionBox({ subscription, planId, planName }) {
  ...
  async function handleCancel() {
    setLoading(true);
    setError("");
    // Paddle is the only payment provider now — confirmed no real
    // customer ever subscribed via PayU or Razorpay before both were
    // removed, so the earlier provider-routing logic here (added when
    // that wasn't yet confirmed) was handling cases that can never
    // actually occur, while still referencing endpoints that no
    // longer exist. Simplified back to a direct call.
