...
import PaddleSubscribeButton from "./PaddleSubscribeButton";
...
export default async function AccountPage({ searchParams }) {
  ...
  const [{ data: profile }, { data: subscriptions }, { data: purchases }, games] = await Promise.all([
    supabase
      .from("profiles")
      .select("username, country, is_admin, is_premium, referral_code, bonus_subscription_until, platform_xp, lifetime_points, avatar_id, theme_id, cosmetic_badge, streak_email_opt_out")
      .eq("id", user.id)
      .single(),
    supabase
      .from("subscriptions")
      .select("status, plan_id, current_period_end, cancel_at_period_end, paddle_customer_id, provider")
      .eq("user_id", user.id),
  ...
  // Admins get every subscriber perk (avatars, themes, cosmetics)
  // without an actual paid subscription — the same bypass used for
  // game access elsewhere on the site, applied here too.
  const isSubscriber = admin || (await hasAnySubscription(supabase, user.id, profile));
  ...
  const tierMap = {};
  // A Paddle customer ID belongs to the person, not to any one
  // plan/subscription — so this looks across all of a user's
  // subscription rows for any that already has one on file, rather
  // than being tied to whichever tier is currently being rendered.
  // Passed to Paddle.js as pwCustomer for Retain; left undefined for
  // a user with no prior Paddle subscription, since Paddle's own docs
  // only cover "no logged-in customer at all" for the empty-object
  // case, not "logged in but no Paddle ID yet" — omitting it entirely
  // is the safer reading for that case.
  const existingPaddleCustomerId = (subscriptions || []).find((s) => s.paddle_customer_id)?.paddle_customer_id;
    ...
    if (g.accessType === "subscription" && g.subscriptionPlanId) {
      if (!tierMap[g.subscriptionPlanId]) {
        tierMap[g.subscriptionPlanId] = {
          ...
          paddlePriceIdMonthly: g.subscriptionPlanPaddlePriceIdMonthly,
          paddlePriceIdAnnual: g.subscriptionPlanPaddlePriceIdAnnual,
          paddlePriceDisplayMonthly: g.subscriptionPlanPaddlePriceDisplayMonthly,
          paddlePriceDisplayAnnual: g.subscriptionPlanPaddlePriceDisplayAnnual,
          count: 0,
        };
      }
  ...
  return (
    <div className="min-h-screen bg-bgDeep text-textLight pb-20 sm:pb-0">
      ...
      <div className="max-w-md mx-auto px-4 sm:px-6 py-10">
                ...
                <div className="rounded-xl border border-accentCyan p-6 bg-bgPanel">
                  ...
                  <div>
                    <PaddleSubscribeButton
                      paddlePriceIdMonthly={tier.paddlePriceIdMonthly}
                      paddlePriceIdAnnual={tier.paddlePriceIdAnnual}
                      priceDisplayMonthly={tier.paddlePriceDisplayMonthly}
                      priceDisplayAnnual={tier.paddlePriceDisplayAnnual}
                      planId={tier.id}
                      planName={tier.name}
                      userId={user.id}
                      userEmail={user.email}
                      paddleCustomerId={existingPaddleCustomerId}
                    />
                  </div>
                </div>
              ) : (
        ...
        <div className="mt-8">
          <h2 className="font-pixel text-sm text-textLight mb-1">SUBSCRIBER PERKS</h2>
          <p className="text-textDim text-sm mb-4">
            Animated avatars, exclusive cosmetics, premium themes, and VIP profile styling — included with any
            active subscription.
          </p>
          <PerksPanel
            ...
            currentBadgeId={profile?.cosmetic_badge}
          />
        </div>
        ...
        <div className="mt-8">
          ...
          <StreakEmailToggle initialOptOut={profile?.streak_email_opt_out} />
        </div>
        ...
        <div className="mt-10 pt-6 border-t border-lineColor text-center font-mono text-[11px] text-textDim flex flex-wrap justify-center gap-4">
          ...
          <Link href="/contact" className="text-accentCyan">Contact</Link>
        </div>
      </div>
    </div>
  );
