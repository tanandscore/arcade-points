...
import PaddleSubscribeButton from "@/app/account/PaddleSubscribeButton";
          ...
          <div className="rounded-xl border border-lineColor p-8 text-center bg-bgPanel">
            ...
            {game.accessType === "onetime" ? (
              <>
                ...
                <p className="text-[12px] text-accentMagenta mb-6 max-w-xs mx-auto">
                  One-time purchases aren't available right now — we're setting this up for our new payment
                  provider. Please check back soon, or contact{" "}
                  <a href="mailto:support@tapandscore.com" className="underline">support@tapandscore.com</a>.
                </p>
              </>
            ) : (
              <>
                <p className="text-textDim mb-1">{game.name} is a {game.subscriptionPlanName || "Power Pass"} game.</p>
                <p className="font-pixel text-xl text-accentAmber mb-2">{game.subscriptionPlanPaddlePriceDisplayMonthly || "—"}/month</p>
                ...
                <PaddleSubscribeButton
                  paddlePriceIdMonthly={game.subscriptionPlanPaddlePriceIdMonthly}
                  paddlePriceIdAnnual={game.subscriptionPlanPaddlePriceIdAnnual}
                  priceDisplayMonthly={game.subscriptionPlanPaddlePriceDisplayMonthly}
                  priceDisplayAnnual={game.subscriptionPlanPaddlePriceDisplayAnnual}
                  planId={game.subscriptionPlanId}
