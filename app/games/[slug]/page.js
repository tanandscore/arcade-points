...
export default async function GamePage({ params }) {
  ...
  const countdownActive = siteSettings?.launch_countdown_at && new Date(siteSettings.launch_countdown_at) > new Date() && !profile?.is_admin;
  ...
  if (!profile?.is_admin) {
    ...
    const activeTournament = tournaments.find((t) => t.status === "active" && t.game_slugs.includes(slug));
    if (activeTournament) {
