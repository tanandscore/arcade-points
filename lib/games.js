...
function shapeGame(row, plansById) {
  ...
  return {
    ...
    // Paddle now offers both monthly and annual billing (added after
    // the first Paddle migration, which only planned for one price
    // per plan) — both null until set in the subscription_plans
    // table (same "no code" editing pattern already used elsewhere
    // in this schema).
    // A plan without these set simply won't show the Paddle option yet.
    subscriptionPlanPaddlePriceIdMonthly: plan?.paddle_price_id_monthly ?? null,
    subscriptionPlanPaddlePriceIdAnnual: plan?.paddle_price_id_annual ?? null,
    subscriptionPlanPaddlePriceDisplayMonthly: plan?.paddle_price_display_monthly ?? null,
    subscriptionPlanPaddlePriceDisplayAnnual: plan?.paddle_price_display_annual ?? null,
    underMaintenance: row.under_maintenance ?? false,
    adminTestOnly: row.admin_test_only ?? false,
  };
}

export async function getGames({ includeTestOnly = false } = {}) {
  const supabase = await createServerSupabase();
  let query = supabase.from("games").select("*").eq("is_active", true).order("sort_order", { ascending: true });
  if (!includeTestOnly) query = query.eq("admin_test_only", false);
  const [{ data: rows, error }, plansById] = await Promise.all([query, getPlansById(supabase)]);
