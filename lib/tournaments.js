...
// Derives the #1 finisher per game from standings already fetched via
// getTournamentStandings — no extra queries, just picks the first
// (already best-sorted) row per game.
export function getTournamentWinners(standings) {
  const winners = {};
  for (const [slug, rows] of Object.entries(standings || {})) {
    if (rows && rows.length > 0) winners[slug] = rows[0];
  }
  return winners;
}

// Finds the (first) currently-active tournament that includes a given
// game slug — used to flash "there's a live tournament" on that
// game's dashboard card and to check the tournament-exclusive lock
// on the game page itself. Takes an already-fetched tournaments list
// rather than querying again, since callers typically already have
// one from getTournaments().
export function findActiveTournamentForGame(tournaments, slug) {
  return (tournaments || []).find((t) => t.status === "active" && (t.game_slugs || []).includes(slug)) || null;
}
