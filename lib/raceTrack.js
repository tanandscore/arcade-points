...
export function buildCar(bodyColor) {
  ...
  // reads as "a real car" rather than a blocky placeholder.
  [-0.85, 0.85].forEach((x) => {
    const mirror = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.12, 0.2), darkMat);
    mirror.position.set(x, 0.68, -0.55);
    group.add(mirror);
  });

  // Exhaust pipes at the rear.
  [-0.35, 0.35].forEach((x) => {
    const pipe = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.25, 8), wheelMat);
    pipe.rotation.x = Math.PI / 2;
    pipe.position.set(x, 0.32, 1.85);
    group.add(pipe);
  });

  // Wheel rims — a thin bright ring on each wheel, gives them a much
  // less flat/plain look up close without adding real geometry cost.
  const rimMat = new THREE.MeshStandardMaterial({ color: 0xc9c9d6, metalness: 0.7, roughness: 0.25 });
  [
    [-0.9, -1.1], [0.9, -1.1], [-0.9, 1.1], [0.9, 1.1],
  ].forEach(([x, z]) => {
    const rim = new THREE.Mesh(new THREE.TorusGeometry(0.2, 0.035, 6, 12), rimMat);
    rim.rotation.y = Math.PI / 2;
    rim.position.set(x, 0.34, z);
    group.add(rim);
  });

  // A faint underglow disc in the car's own color — a small racing-
  // game flourish, and it also makes the car easier to spot against
  // the dark road at a glance.
  const glow = new THREE.Mesh(
    new THREE.CircleGeometry(1.1, 16),
    new THREE.MeshBasicMaterial({ color: bodyColor, transparent: true, opacity: 0.16 })
  );
...
// A distant ring of low-poly hills around the track — cheap (a
// handful of cones), but it's what turns a flat horizon into a place
// that feels like it has actual depth and distance to it.
export function buildDistantHills(count = 18, radius = 70) {
  const group = new THREE.Group();
  const hillMat = new THREE.MeshStandardMaterial({ color: 0x1a1042, roughness: 1 });
  for (let i = 0; i < count; i++) {
    const angle = (i / count) * Math.PI * 2 + Math.random() * 0.15;
    const dist = radius + Math.random() * 25;
    const height = 10 + Math.random() * 16;
    const hill = new THREE.Mesh(new THREE.ConeGeometry(10 + Math.random() * 8, height, 7), hillMat);
    hill.position.set(Math.cos(angle) * dist, height / 2 - 2, Math.sin(angle) * dist);
    group.add(hill);
  }
  return group;
}

// A start/finish gate arching over the track — a genre-standard
// landmark that also gives players a clear visual anchor for where a
// lap begins and ends.
export function buildFinishArch(accentColor) {
  const group = new THREE.Group();
  const postMat = new THREE.MeshStandardMaterial({ color: 0x2a1a55, metalness: 0.3, roughness: 0.5 });
  const bannerMat = new THREE.MeshStandardMaterial({ color: accentColor, emissive: accentColor, emissiveIntensity: 0.5 });
  ...
  const banner = new THREE.Mesh(new THREE.BoxGeometry(TRACK_WIDTH + 2.5, 0.9, 0.3), bannerMat);
