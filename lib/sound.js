...
export const sfx = {
  ...
  // Distinct per-weapon-class firing sounds for Operation Blacksite —
  // previously every weapon shared the same generic hit() sound
  // regardless of type. Pistol: sharp, high, quick. SMG: buzzy,
  // lower-volume (it fires fast, so each individual shot needs to sit
  // back in the mix or a sustained burst becomes overwhelming
  // noise). Rifle: deep, punchy, the most powerful-sounding of the
  // three — matches it also dealing the most damage per shot.
  pistolShot: () => tone({ frequency: 900, duration: 0.08, type: "square", glideTo: 320, volume: 0.34 }),
  smgShot: () => tone({ frequency: 480, duration: 0.05, type: "sawtooth", glideTo: 200, volume: 0.24 }),
  rifleShot: () => tone({ frequency: 220, duration: 0.1, type: "square", glideTo: 75, volume: 0.4, layered: true }),
  // A real, dedicated reload sound — previously tryReload() just
  // played the same generic menu-select click every other UI action
  // uses, not a weapon sound at all. This is a genuine three-stage
  // sequence (mag drop, new mag seated, chamber racked) rather than
  // a single beep, at a volume that actually reads as a mechanical
  // action during a firefight rather than disappearing under it.
  weaponReload: () =>
    sequence([
      { frequency: 180, duration: 0.05, type: "square", volume: 0.3, gap: 100 },
      { frequency: 140, duration: 0.06, type: "square", volume: 0.32, gap: 130 },
      { frequency: 420, duration: 0.05, type: "square", volume: 0.34 },
    ]),
  // A sharp accent tone playing alongside the spoken "Headshot!"
  // callout (see speak() in OperationBlacksite.js) — the tone lands
  // instantly, the voice line follows a beat behind, together making
  // the moment land immediately rather than waiting on TTS latency.
  headshotHit: () => tone({ frequency: 1200, duration: 0.14, type: "triangle", glideTo: 1900, volume: 0.2, layered: true }),
...
// Combat mood — a genuinely different piece, not the day/night loop
// turned up: a driving, near-constant low bass pulse (no rests
// between notes) plus a tense, lower-register lead built on a
// dissonant half-step, for the dramatic, cinematic urgency a bandit
// raid actually calls for. Triggered from the game whenever bandits
// are on the field.
const KINGDOM_COMBAT_BASS = [110, 110, 0, 110, 116.54, 116.54, 0, 116.54, 110, 110, 0, 110, 98, 98, 0, 98];
const KINGDOM_COMBAT_LEAD = [220, 0, 233.08, 0, 220, 0, 207.65, 0, 220, 0, 233.08, 0, 246.94, 233.08, 220, 0];
const KINGDOM_TARGET_GAIN = 0.5;
const KINGDOM_COMBAT_GAIN = 0.62; // louder still during a raid — the whole point is that it should feel more urgent, not just different
...
export function createKingdomMusic() {
  ...
  function schedule() {
    while (nextStepTime < ctx.currentTime + 0.15) {
      const bassSeq = mood === "combat" ? KINGDOM_COMBAT_BASS : mood === "night" ? KINGDOM_NIGHT_BASS : KINGDOM_DAY_BASS;
      const leadSeq = mood === "combat" ? KINGDOM_COMBAT_LEAD : mood === "night" ? KINGDOM_NIGHT_LEAD : KINGDOM_DAY_LEAD;
      const step = stepIndex % bassSeq.length;
      // Combat hits harder per-note too, not just a louder master
      // gain — a real dynamic difference, not a volume knob.
      const bassPeak = mood === "combat" ? 0.3 : 0.22;
      const leadPeak = mood === "combat" ? 0.22 : 0.16;
      playNote(nextStepTime, bassSeq[step], "sine", bassPeak, KINGDOM_STEP * 1.6);
      playNote(nextStepTime, leadSeq[step], mood === "combat" ? "sawtooth" : "triangle", leadPeak, KINGDOM_STEP * 1.3);
      stepIndex += 1;
  ...
  let lastKnownMuted = null;
  function targetGain() {
    return mood === "combat" ? KINGDOM_COMBAT_GAIN : KINGDOM_TARGET_GAIN;
  }
  function applyMuteState() {
    if (stopped || ctx.state === "closed") return;
    ensureInitialized();
    ...
    if (muted) {
      ...
      if (ctx.state === "suspended") ctx.resume();
      masterGain.gain.setTargetAtTime(targetGain(), ctx.currentTime, 0.5);
    }
  }
  const pollId = setInterval(applyMuteState, 400);
  applyMuteState();
  ...
  return {
    // Called whenever the game's day-night phase changes, or a raid
    // starts/ends — swaps which note sequences the scheduler is
    // pulling from on the fly, no restart or gap needed. Also
    // re-applies gain immediately so combat's louder mix kicks in the
    // instant bandits appear, not on the next mute-poll cycle.
    setMood(newMood) {
      if (mood === newMood) return;
      mood = newMood;
      if (!stopped && !muted) masterGain.gain.setTargetAtTime(targetGain(), ctx.currentTime, 0.6);
    },
...
// Buy-phase music for Operation Blacksite — an original, deliberately
// bass-heavy composition, not a reproduction of any existing song.
// The brief was "a mood like Down With the Sickness or Riders on the
// Storm, more bass" — read as a request for that kind of heavy,
// driving-yet-atmospheric tension, not a request to recreate either
// song's actual melody or chord progression, which this doesn't do.
// Built from two real, distinct techniques for genuinely heavier low
// end (not just a louder mix): a sub-bass layer one full octave below
// the main bassline, and a deep, pitch-dropping kick drum, both
// getting more headroom in the gain balance than any other layer.
// Sparse, moody stabs sit on top for tension rather than a full lead
// melody, echoing the atmospheric side of the brief without echoing
// any specific song's actual notes.
const BUY_SUBBASS = [20.6, 20.6, 0, 20.6, 20.6, 0, 20.6, 0, 23.12, 23.12, 0, 23.12, 20.6, 0, 19.45, 0];
const BUY_BASS = [41.2, 41.2, 0, 41.2, 41.2, 0, 41.2, 0, 46.25, 46.25, 0, 46.25, 41.2, 0, 38.89, 0];
const BUY_STAB = [0, 0, 0, 0, 0, 0, 0, 0, 164.81, 0, 0, 0, 0, 0, 155.56, 0];
const BUY_TEMPO = 92;
const BUY_STEP = 60 / BUY_TEMPO / 2;
const BUY_TARGET_GAIN = 0.62;

export function createBuyPhaseMusic() {
  const noop = { resumeIfNeeded() {}, stop() {} };
  if (typeof window === "undefined") return noop;
  const Ctx = window.AudioContext || window.webkitAudioContext;
  if (!Ctx) return noop;
  ...
  // A deep, pitch-dropping thump — real low-end weight from the
  // envelope shape, not just a low frequency.
  function playKick(time) {
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = "sine";
  ...
  function schedule() {
    while (nextStepTime < ctx.currentTime + 0.15) {
      const step = stepIndex % BUY_BASS.length;
      // Sub-bass and main bass both play the driving line, one
      // octave apart — this is what makes it read as genuinely
      // heavier rather than just turned up.
      playNote(nextStepTime, BUY_SUBBASS[step], "sine", 0.42, BUY_STEP * 1.7);
      playNote(nextStepTime, BUY_BASS[step], "sawtooth", 0.3, BUY_STEP * 1.1);
      if (BUY_STAB[step]) {
        playNote(nextStepTime, BUY_STAB[step], "triangle", 0.16, BUY_STEP * 3.5);
      }
      if (step % 8 === 0) playKick(nextStepTime);
      if (step % 8 === 4) playKick(nextStepTime);
      stepIndex += 1;
      nextStepTime += BUY_STEP;
    }
  }
  const schedulerId = setInterval(schedule, 50);
  schedule();
  ...
  function applyMuteState() {
    ...
    if (muted) {
      ...
      if (ctx.state === "suspended") ctx.resume();
      masterGain.gain.setTargetAtTime(BUY_TARGET_GAIN, ctx.currentTime, 0.5);
    }
  }
  const pollId = setInterval(applyMuteState, 400);
  applyMuteState();
  ...
  return {
    resumeIfNeeded() {
      if (stopped) return;
      if (ctx.state === "suspended") ctx.resume();
      if (!muted) masterGain.gain.setTargetAtTime(BUY_TARGET_GAIN, ctx.currentTime, 0.4);
    },
    stop() {
      if (stopped) return;
      stopped = true;
