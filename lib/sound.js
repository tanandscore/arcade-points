...
// A separate, more intense track for Titan Arena specifically — a
// driving E-minor progression (Em–C–D–Em, the classic "epic battle"
// chord pattern) at a faster tempo than the general arcade music,
// with an added low war-drum hit on the downbeat. Same lookahead-
// scheduler approach as createArcadeMusic, so timing stays tight.
const BATTLE_BASS = [82.41, 82.41, 0, 82.41, 130.81, 130.81, 0, 130.81, 146.83, 146.83, 0, 146.83, 82.41, 82.41, 0, 82.41];
const BATTLE_LEAD = [
...
const BATTLE_TEMPO = 140;
const BATTLE_STEP = 60 / BATTLE_TEMPO / 2;
const BATTLE_TARGET_GAIN = 0.5;
...
export function createBattleMusic() {
  ...
  function schedule() {
    while (nextStepTime < ctx.currentTime + 0.15) {
      const step = stepIndex % BATTLE_BASS.length;
      playNote(nextStepTime, BATTLE_BASS[step], "sawtooth", 0.3, BATTLE_STEP * 0.85);
      playNote(nextStepTime, BATTLE_LEAD[step], "square", 0.2, BATTLE_STEP * 0.7);
      if (step % 4 === 0) {
        playNote(nextStepTime, 55, "sine", 0.35, 0.12); // war-drum thump on the downbeat
      }
      stepIndex += 1;
      nextStepTime += BATTLE_STEP;
    }
  }
  const schedulerId = setInterval(schedule, 50);
  schedule();
  ...
  function applyMuteState() {
    ...
    if (muted) {
      ...
      if (ctx.state === "suspended") ctx.resume();
      masterGain.gain.setTargetAtTime(BATTLE_TARGET_GAIN, ctx.currentTime, 0.5);
    }
  }
  const pollId = setInterval(applyMuteState, 400);
  applyMuteState();
  ...
  return {
    resumeIfNeeded() {
      if (stopped) return;
      if (ctx.state === "suspended") ctx.resume();
      if (!muted) masterGain.gain.setTargetAtTime(BATTLE_TARGET_GAIN, ctx.currentTime, 0.4);
    },
    stop() {
      if (stopped) return;
      stopped = true;
