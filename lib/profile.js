...
export async function getPublicProfile(username) {
  ...
  const { data: profile } = await service
    .from("profiles")
    .select("id, username, platform_xp, lifetime_points, longest_streak, created_at, avatar_id, theme_id, cosmetic_badge, bonus_subscription_until")
    .eq("username", username)
    .maybeSingle();
  if (!profile) return null;

  const [{ data: scoreRows }, { data: recentRows }, rankResult, streakDays, activeSeason, isSubscriber] = await Promise.all([
    ...
    hasAnySubscription(service, profile.id, profile),
  ]);
  ...
  return {
    ...
    cosmeticBadge: profile.cosmetic_badge || null,
  };
}
