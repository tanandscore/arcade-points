...
const DIFFICULTY = {
  1: { label: "CADET", mult: 1, reinforceMult: 0.6, attackThreshold: 4 },
  2: { label: "EASY", mult: 1.2, reinforceMult: 0.8, attackThreshold: 3.5 },
  3: { label: "MEDIUM", mult: 1.45, reinforceMult: 1, attackThreshold: 3 },
  4: { label: "HARD", mult: 1.75, reinforceMult: 1.25, attackThreshold: 2.5 },
  5: { label: "ACE", mult: 2.1, reinforceMult: 1.5, attackThreshold: 2 },
  6: { label: "LEGEND", mult: 2.5, reinforceMult: 1.8, attackThreshold: 1.5 },
};
const POSITIONS = {
  A: { top: "10%", left: "10%" },
  B: { top: "10%", left: "55%" },
...
function aiTurn(state, diff) {
  let next = { owner: { ...state.owner }, troops: { ...state.troops } };
  const aiTerritories = ownedBy(next, "ai");
  // reinforce a random owned territory — scaled by difficulty
  if (aiTerritories.length) {
    const t = aiTerritories[Math.floor(Math.random() * aiTerritories.length)];
    next.troops[t] += Math.max(1, Math.round(aiTerritories.length * diff.reinforceMult));
  }
  // attack a random reachable weaker neighbor — lower threshold means
  // the AI is willing to attack even fights it's less sure to win
  for (const source of aiTerritories) {
    ...
    if (targets.length && next.troops[source] > diff.attackThreshold) {
...
export default function EmpireCommand({ onFinish, accentColor }) {
  ...
  function finishGame(finalState) {
    ...
    const score = Math.round((territories * 100 + troopTotal * 5) * DIFFICULTY[levelRef.current].mult);
    setTimeout(() => onFinish(score), 800);
  }

  function endRound(currentState) {
    const afterAI = aiTurn(currentState, DIFFICULTY[levelRef.current]);
    setState(afterAI);
    setSelected(null);
  ...
  return (
    <div className="text-center">
      <p className="font-mono text-xs mb-2 text-textDim">
        Round {Math.min(round, TOTAL_ROUNDS)} of {TOTAL_ROUNDS} · Lvl {level}
      </p>
