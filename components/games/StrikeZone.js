...
export default function StrikeZone({ onFinish, accentColor }) {
  ...
  function startSpawning(rate) {
    ...
    spawnRef.current = setInterval(() => {
      setTargets((prev) => (prev.length < maxActiveRef.current ? [...prev, randomTarget()] : prev));
    }, rate);
  }
  ...
  function handleHit(target) {
    ...
    if (target.isHostile) {
      ...
      if (newLevel > levelRef.current) {
        ...
        setLevel(newLevel);
        maxActiveRef.current = Math.min(5, 2 + Math.floor(newLevel / 2));
        startSpawning(Math.max(280, 550 - newLevel * 35));
    ...
    return (
      <div className="text-center">
        <p className="mb-6 text-textDim">
          ...
          you. Every {HITS_PER_LEVEL} hits is a level: more targets, faster spawns, bonus time.
        </p>
  ...
  return (
    <div className="text-center relative">
      ...
      <div className="flex justify-between font-mono text-xs mb-4 text-textDim">
        <span>
          Hits: <span className="text-textLight">{hits}</span> · Lvl {level} · Civilians hit:{" "}
