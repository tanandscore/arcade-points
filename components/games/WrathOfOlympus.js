    ...
    <div className="text-center relative" ref={wrapRef}>
      ...
      >
        ...
        {!isFullscreen && (
          ...
          // called on game start, but browsers can silently reject it
          // for reasons outside this component's control (notably,
          // being embedded in an iframe without an explicit fullscreen
          // permission on that iframe itself). Rather than leave the
          // player stuck at the smaller fallback size with no
          // recourse, a direct, real user click here gives fullscreen
          // every browser's normal chance to succeed.
          <button
            onClick={() => {
              const el = wrapRef.current?.ownerDocument?.documentElement;
              if (el?.requestFullscreen) el.requestFullscreen().catch(() => {});
