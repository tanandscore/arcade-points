...
const DURATION = 75;
const BUILDINGS_PER_LEVEL = 4;
const LEVEL_BONUS_SECONDS = 10;
...
export default function ColonyRush({ onFinish, accentColor }) {
  ...
  function build(type) {
    ...
    const nextBuildings = { ...buildings, [type]: buildings[type] + 1 };
    setBuildings(nextBuildings);
    if (type === "market") setBonusScore((s) => s + 50);

    const totalBuildings = nextBuildings.house + nextBuildings.farm + nextBuildings.sawmill + nextBuildings.market;
    const newLevel = Math.floor(totalBuildings / BUILDINGS_PER_LEVEL) + 1;
    if (newLevel > levelRef.current) {
      levelRef.current = newLevel;
      setLevel(newLevel);
      setBonusScore((s) => s + 30 * newLevel);
    ...
    return (
      <div className="text-center">
        <p className="mb-6 text-textDim">
          ...
          score before time runs out. Every {BUILDINGS_PER_LEVEL} buildings is a level and earns bonus time.
        </p>
  ...
  return (
    <div className="relative">
      ...
      <div className="flex justify-between font-mono text-xs mb-4 text-textDim">
        <span>
          🪵 <span className="text-textLight">{Math.floor(wood)}</span> · 🌾 <span className="text-textLight">{Math.floor(food)}</span> · Lvl {level}
        </span>
