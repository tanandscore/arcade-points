...
const WATER = { x: 730, y: 0, w: 90, h: MAP_H };
...
export default function KingdomsOfAsh({ onFinish, accentColor }) {
  ...
  function handleCanvasClick(e) {
    ...
    if (!canPlace(type, x, y)) {
      setNotice(def.needsForest ? "Lumber camps must be near a forest" : def.needsWater ? "Fishing docks must be near the water" : "Can't build there");
  ...
  function begin(resumeSave) {
    ...
    simIntervalRef.current = setInterval(() => {
      ...
      if (musicRef.current) musicRef.current.setMood(nightForSpawning ? "night" : "day");
