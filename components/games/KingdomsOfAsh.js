  ...
  return (
    <div className="text-center relative">
      ...
      >
        ...
        {!isFullscreen && (
          // A real fallback, not decoration — requestFullscreen() is
          // called in begin(), but browsers can silently reject it
          // for reasons outside this component's control (notably,
          // being embedded in an iframe without an explicit
          // fullscreen permission on that iframe itself). A direct,
          // real user click here gives fullscreen every browser's
          // normal chance to succeed.
          <button
            onClick={() => {
              const el = document.documentElement;
              if (el.requestFullscreen) el.requestFullscreen().catch(() => {});
            }}
            className="absolute top-2 right-2 font-mono text-[10px] px-2.5 py-1.5 rounded-md z-10 bg-bgDeep/90 border border-amber-600/50 text-textDim"
          >
            ⛶ Fullscreen
          </button>
        )}
