...
export default function ApexCircuit({ onFinish, accentColor }) {
  ...
  // Runs AFTER "started" flips true and React has actually rendered
  // the canvas container — doing this setup synchronously inside the
  // Start button's click handler was the bug: at that exact moment
  // the container div hadn't rendered yet (it only appears once
  // started is true), so mountRef.current was still null and the
  ...
  useEffect(() => {
    ...
    sceneRef.current = { renderer, scene, camera, carGroup, boostPads };
