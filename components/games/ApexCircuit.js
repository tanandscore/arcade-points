...
const CORNER_STRESS_LIMIT = 1.55;
...
export default function ApexCircuit({ onFinish, accentColor }) {
  ...
  function finish() {
    ...
    sfx.lose();
    const score = Math.round(stateRef.current.totalProgress * 900) + stateRef.current.boosts * 40 + stateRef.current.lapBonus;
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      // Steering rotates the car's own heading — the car does NOT
      // automatically follow the track's curve anymore. Don't turn,
      // and you drive straight off it, exactly like a real car.
      const turnRate = 0.064 - speedRatio * 0.016;
      ...
      const curvatureAhead = trackCurvatureAt(curve, nearest.t + 0.012);
      ...
      if (cornerStress > CORNER_STRESS_LIMIT) {
        moveSpeed *= 0.7;
        s.heading += (Math.random() - 0.5) * 0.025;
      }
      ...
      s.lastT = nearest.t;
      const newLap = Math.max(1, Math.floor(s.totalProgress) + 1);
      if (newLap > lapRef.current) {
        lapRef.current = newLap;
        s.lapBonus += 60 * newLap;
        sfx.levelUp();
        haptics.success();
    ...
    return (
      <div className="text-center">
        ...
        <div className="text-left max-w-xs mx-auto mb-6 space-y-2 text-sm text-textDim">
          <p>🏎️ You control both speed and steering. Nothing is automatic — if you don't turn, you drive straight, not along the track.</p>
          <p>▲ Accelerate, ▼ Brake — arrow keys or the on-screen pedals.</p>
          <p>◀ ▶ Steer left/right — arrow keys or the on-screen buttons.</p>
          <p>🛣️ Stay on the road between the glowing dashed lines. Drift onto the grass or gravel and you'll slow down hard.</p>
          <p>🌀 A real circuit: sweeping curves, a chicane, a tight hairpin, and long straights. Brake before sharp turns.</p>
          <p>⚡ Drive through gold orbs for a boost. Every completed lap earns a bonus. 60 seconds — cover as much distance as you can.</p>
        </div>
  ...
  return (
    <div className="text-center">
      ...
      {warnCorner && <p className="font-mono text-[10px] text-accentMagenta mb-2 ap-blink">⚠️ Sharp corner ahead — brake and steer!</p>}
      <div className="relative">
        {levelUpFlash && (
          <div className="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
            <p className="font-pixel text-sm text-accentAmber ap-blink bg-bgDeep/80 px-4 py-2 rounded-lg">LAP {lap}!</p>
          </div>
        )}
        <div
          ref={mountRef}
