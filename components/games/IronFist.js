...
function opponentDamageMult(level) {
...
export default function IronFist({ onFinish, accentColor }) {
  ...
  function finishRun() {
    sfx.lose();
    setTimeout(() => onFinish(Math.max(0, runScoreRef.current)), 900);
  }
  ...
  function advanceOpponent() {
    // Beating an opponent doesn't end the run — a tougher rival steps
    // up next, score carries forward, until you actually lose a match.
    runScoreRef.current += 150 * opponentLevelRef.current;
    setRunScore(runScoreRef.current);
    opponentLevelRef.current += 1;
    setOpponentLevel(opponentLevelRef.current);
    setMyWins(0);
    setAiWins(0);
    newRound();
    sfx.levelUp();
  ...
  function exchange(stance) {
    ...
    if (beats(stance, aiStance)) {
      ...
      const dmg = Math.round(BASE_DAMAGE * opponentDamageMult(opponentLevelRef.current));
      newMyHp = Math.max(0, myHp - dmg);
      newCombo = 0;
      sfx.hit();
    ...
    if (newAiHp <= 0 || newMyHp <= 0) {
      ...
      if (nextAiWins >= ROUNDS_TO_WIN) {
        setLog("Defeated.");
        setGameOver(true);
        finishRun();
      } else if (nextMyWins >= ROUNDS_TO_WIN) {
        setTimeout(() => {
          advanceOpponent();
          setLog(`Opponent ${opponentLevelRef.current} defeated! Next rival stepping up.`);
          setBusy(false);
        }, 900);
        return;
      } else {
    ...
    return (
      <div className="text-center">
        <p className="mb-6 text-textDim">
          ...
          and counter. Land 4 hits in a row to unlock a Finishing Strike. Beat an opponent (best of 3 rounds) and a
          tougher rival steps up immediately — your score keeps climbing until you actually lose a match.
        </p>
    ...
    <div className="text-center relative">
      {levelUpFlash && (
        <div className="absolute inset-x-0 -top-2 z-10 flex items-center justify-center pointer-events-none">
          <p className="font-pixel text-sm text-accentAmber ap-blink bg-bgDeep/80 px-4 py-2 rounded-lg">OPPONENT {opponentLevel}!</p>
        </div>
      )}
      <p className="font-mono text-xs mb-3 text-textDim">
        Rival {opponentLevel} · Wins — You: {myWins} · Opponent: {aiWins} · Score: {runScore}
      </p>
