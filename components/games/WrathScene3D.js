...
export default function WrathScene3D({ mapW, mapH, temple, altar, championsRef, enemiesRef, wardsRef, enemyColors, onWorldClick }) {
  ...
  useEffect(() => {
    ...
    // A real, confirmed bug fixed here: window.resize only fires when
    // the browser window itself changes size — it does NOT fire for
    // the container's own size changing due to CSS layout settling
    // after mount, or for fullscreen transitions (the Fullscreen API
    // can resize an element without the window itself resizing, and
    // this is not reliable across browsers). The camera/renderer were
    // being initialized once with whatever size happened to be
    // available at that exact moment, then never corrected — exactly
    // what produced a tiny, dark render inside a much larger visible
    // box. ResizeObserver watches the actual container element
    // directly, for any reason its size changes, not just window
    // resizes.
