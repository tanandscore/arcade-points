...
export default function PeakAscent({ onFinish, accentColor }) {
  ...
  function finish() {
    ...
    sfx.lose();
    onFinish(Math.round(heightRef.current));
  }
    ...
    return (
      <div className="text-center">
        <p className="mb-6 text-textDim">
          Climb the ladders as high as you can. Barrels roll down every lane — switch lanes to dodge them. Height is
          your score. 3 lives.
        </p>
  ...
  return (
    <div className="text-center">
      <div className="flex justify-between font-mono text-xs mb-3 text-textDim">
        <span>Height: <span className="text-textLight">{Math.round(height)}m</span></span>
