...
export default function Dominion({ onFinish, accentColor }) {
  ...
  // Runs AFTER "started" flips true and the canvas container has
  // actually rendered — building the scene synchronously inside the
  // Start button's click handler was the bug: at that moment the
  // container div didn't exist yet, so mountRef.current was null and
  // clicking Start did nothing at all.
  useEffect(() => {
