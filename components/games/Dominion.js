...
import { sfx, createAmbientLoop } from "@/lib/sound";
...
const DIFFICULTY = {
  1: { label: "CADET", mult: 1, aiGoldBase: 4, attackRisk: 1.15 },
  2: { label: "EASY", mult: 1.2, aiGoldBase: 5, attackRisk: 1.05 },
  3: { label: "MEDIUM", mult: 1.45, aiGoldBase: 6, attackRisk: 0.95 },
  4: { label: "HARD", mult: 1.75, aiGoldBase: 7, attackRisk: 0.85 },
  5: { label: "ACE", mult: 2.1, aiGoldBase: 8, attackRisk: 0.75 },
  6: { label: "LEGEND", mult: 2.5, aiGoldBase: 10, attackRisk: 0.65 },
};
...
export default function Dominion({ onFinish, accentColor }) {
  ...
  function finishGame() {
    ...
    score = Math.round(score * DIFFICULTY[levelRef.current].mult);
  ...
  function aiTurn() {
    ...
    const diff = DIFFICULTY[levelRef.current];
    let aiGold = diff.aiGoldBase; // AI's own simplified gold pool, independent of the player's
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (const source of attackers) {
        ...
        for (const target of targets) {
          const defense = target.troops * (target.fort ? 1.5 : 1);
          if (defense < source.troops * diff.attackRisk) {
  ...
  useEffect(() => {
    ...
    const ambientMusic = createAmbientLoop();
  ...
  return (
    <div className="text-center">
      <div className="flex justify-between font-mono text-xs mb-2 text-textDim">
        <span>Round {Math.min(round, ROUNDS)} / {ROUNDS} · Lvl {level}</span>
