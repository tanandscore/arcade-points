...
const WALLS = [
  { x: 0, y: 0, w: MAP_W, h: 14 },
  { x: 0, y: MAP_H - 14, w: MAP_W, h: 14 },
  { x: 0, y: 0, w: 14, h: MAP_H },
  { x: MAP_W - 14, y: 0, w: 14, h: MAP_H },
  { x: 220, y: 60, w: 22, h: 150 },
  { x: 460, y: 60, w: 22, h: 150 },
  { x: 90, y: 260, w: 160, h: 22 },
  { x: 440, y: 300, w: 180, h: 22 },
  { x: 320, y: 180, w: 60, h: 22 },
];

const SITE_A = { x: 560, y: 90, r: 46 };
const SITE_B = { x: 110, y: 400, r: 46 };
const PLAYER_SPAWN = { x: 60, y: 60 };
const BOT_SPAWNS = [
  { x: 610, y: 130 },
  { x: 150, y: 360 },
  { x: 350, y: 400 },
...
export default function OperationBlacksite({ onFinish, accentColor }) {
  ...
  function resetForRound() {
    playerRef.current = { x: PLAYER_SPAWN.x, y: PLAYER_SPAWN.y, aim: 0, hp: 100, armor: buyState.armor ? 1 : 0, weapon: buyState.weapon, ammo: WEAPONS[buyState.weapon].mag, reserve: WEAPONS[buyState.weapon].mag * 3, reloadingUntil: 0, lastShotAt: 0, heat: 0, alive: true, planting: false, plantStart: 0, defusing: false, defuseStart: 0 };
    botsRef.current = BOT_SPAWNS.map((s, i) => ({ id: i, x: s.x, y: s.y, hp: 80, state: "patrol", patrolTarget: { x: s.x + (Math.random() - 0.5) * 80, y: s.y + (Math.random() - 0.5) * 80 }, aim: 0, alertUntil: 0, lastShotAt: 0, alive: true }));
  ...
  function finishMatch(won) {
    ...
    setPhase("over");
    const score = roundsWonRef.current * 150 + Math.round(moneyRef.current / 20);
  ...
  function tryFire(now) {
    ...
    let closestHit = segmentHitsWall(p.x, p.y, angle, BULLET_RANGE, WALLS);
  ...
  function botAI(b, now, p) {
    ...
    const canSeePlayer = p.alive && dToPlayer < 260 && !walkingSilentRef.current;
    ...
    if (b.state === "engage" && p.alive) {
      ...
      if (now - b.lastShotAt > 420 + Math.random() * 300) {
        b.lastShotAt = now;
        const spread = 0.09;
        const angle = b.aim + (Math.random() - 0.5) * spread;
        const hitPlayer = segmentHitsWall(b.x, b.y, angle, dToPlayer + 4, WALLS);
        if (!hitPlayer) {
          const dmg = Math.round(14 * (1 - (p.armor ? ARMOR_REDUCTION : 0)));
    ...
    simIntervalRef.current = setInterval(() => {
      ...
      if (p.alive) {
        ...
        if (mv.x || mv.y) {
          ...
          const blockedX = WALLS.some((w) => nx > w.x - 10 && nx < w.x + w.w + 10 && p.y > w.y - 10 && p.y < w.y + w.h + 10);
          const blockedY = WALLS.some((w) => p.x > w.x - 10 && p.x < w.x + w.w + 10 && ny > w.y - 10 && ny < w.y + w.h + 10);
        ...
        const nearA = dist(p.x, p.y, SITE_A.x, SITE_A.y) < SITE_A.r;
        const nearB = dist(p.x, p.y, SITE_B.x, SITE_B.y) < SITE_B.r;
    ...
    return (
      <div className="text-center max-w-sm mx-auto">
        ...
        <p className="text-textDim text-sm mb-6">
          ...
          of 5 rounds — plant the bomb at Site A or B and defend it, or eliminate every defender. Buy weapons and
          armor between rounds with money earned from kills and wins.
        </p>
  ...
  return (
    <div className="text-center relative" ref={wrapRef}>
      <div className="flex justify-between font-mono text-[11px] mb-2 text-textDim max-w-[700px] mx-auto">
        <span>Rounds {hud.roundsWon}-{hud.roundsLost}</span>
