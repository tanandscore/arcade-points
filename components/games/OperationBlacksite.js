...
// Real teammate identity — previously both AI teammates were
// interchangeable, unnamed, dealt identical flat damage regardless
// of range, and used a hardcoded tracer color unrelated to any
// actual weapon. Now each gets a real name (persisted for the whole
// match, not reassigned every round) and one of the two non-pistol
// weapons for whichever side is currently active — one close-range
// specialist, one longer-range one, a real squad composition rather
// than two clones of each other.
const TEAMMATE_NAMES = ["Reyes", "Kade", "Voss", "Milo", "Osei", "Brandt", "Iker", "Suri"];
function teammateWeaponForSide(side, index) {
  if (side === "police") return index === 0 ? "smgPolice" : "riflePolice";
  return index === 0 ? "smg" : "rifle";
}
...
export default function OperationBlacksite({ onFinish, accentColor }) {
  ...
  const teammateNamesRef = useRef(["Reyes", "Kade"]); // assigned fresh at match start, persists across every round of that match
  ...
  // Real match-stats tracking — previously nothing at all was
  // counted (no kills, no headshots, no plants/defuses), so there
  // was no real data to ever show a post-match breakdown from.
  const playerKillsRef = useRef(0);
  const playerHeadshotsRef = useRef(0);
  const playerPlantsRef = useRef(0);
  const playerDefusesRef = useRef(0);
  const teammateKillsRef = useRef({ "mate-0": 0, "mate-1": 0 });
  ...
  function resetForRound() {
    ...
    teammatesRef.current = [0, 1].map((i) => ({
      id: `mate-${i}`,
      name: teammateNamesRef.current[i] || `Teammate ${i + 1}`,
      weapon: teammateWeaponForSide(playerSideRef.current, i),
      x: spawn.x + (i === 0 ? -26 : 26),
      y: spawn.y + 18,
      hp: 100,
      aim: 0,
  ...
  function startMatch() {
    ...
    // Two distinct names for the whole match — shuffled and sliced
    // rather than two independent random picks, which could
    // otherwise land on the same name for both teammates.
    const shuffledNames = [...TEAMMATE_NAMES].sort(() => Math.random() - 0.5);
    teammateNamesRef.current = shuffledNames.slice(0, 2);
    playerKillsRef.current = 0;
    playerHeadshotsRef.current = 0;
    playerPlantsRef.current = 0;
    playerDefusesRef.current = 0;
    teammateKillsRef.current = { "mate-0": 0, "mate-1": 0 };
  ...
  function finishMatch(won) {
    ...
    finalScoreRef.current = Math.max(0, score);
    fetch("/api/difficulty-scores", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ game: "operationblacksite", difficulty: difficultyRef.current, score }),
    }).catch(() => {});
    // The score submits immediately as before — only the exit itself
    // now waits for the player to actually look at the stats
    // breakdown and dismiss it, instead of an automatic 1.5s timeout
    // that gave a real, newly-built stats screen no time to matter.
  }
    ...
    for (const t of teammatesRef.current) {
      ...
      if (canEngage) {
        ...
        if (now - t.lastShotAt > 480 + Math.random() * 260) {
          ...
          if (!hitObstruction) {
            // Real weapon-based damage instead of a flat 13-17
            // regardless of what either teammate was actually
            // carrying — reuses the exact same range-falloff formula
            // the player's own shots use, so a teammate's SMG and
            // rifle genuinely behave differently, not just visually.
            const weaponDef = WEAPONS[t.weapon];
            const dmg = Math.round(weaponDef.damage * rangeDamageMult(weaponDef, bestD));
            nearestEnemy.hp -= dmg;
            spawnParticles(nearestEnemy.x, nearestEnemy.y, "#ffe14d", 6);
            bulletTracersRef.current.push({ x0: t.x, y0: t.y, x1: nearestEnemy.x, y1: nearestEnemy.y, color: weaponDef.tracer, life: 1 });
            if (nearestEnemy.hp <= 0 && nearestEnemy.alive) {
              ...
              teammateKillsRef.current[t.id] = (teammateKillsRef.current[t.id] || 0) + 1;
            }
          }
        }
        if (bestD < 100) {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      [mapRef.current.siteA, mapRef.current.siteB].forEach((site, i) => {
        ...
        // A real, separate pre-existing bug found while working on
        // something else: this never set textAlign itself, and its
        // manual "-18" offset only makes sense assuming the canvas
        // default (left-aligned) — but canvas context state isn't
        // reset between frames, so any other text drawn earlier that
        // set textAlign to "center" (several places in this file do)
        // would leave these site labels silently shifted for every
        // frame after that, not just the first one. Fixed by setting
      ...
      for (const t of teammatesRef.current) {
        ...
        // A real name label — teammates previously had no visible
        // identity at all beyond an unlabeled colored dot. Wrapped in
        // save/restore so the textAlign change here doesn't leak into
        // other text drawn later in this same frame or persist into
        // the next one — canvas context state isn't automatically
        // reset between frames, and at least one other label
        // elsewhere in this file (the site markers) relies on the
        // default left alignment rather than setting its own.
        ctx.save();
  ...
  if (phase === "over") {
    // Real match-stats breakdown — previously this screen showed only
    // the win/loss result and the round score, then auto-exited after
    // 1.5 seconds regardless of whether the player had time to read
    // any of it. Nothing about individual performance (kills,
    // headshots, plants, defuses) was ever tracked at all before this,
    // so there was no real data to show here even if there had been
    // room for it.
    const names = ["You", teammateNamesRef.current[0] || "Teammate 1", teammateNamesRef.current[1] || "Teammate 2"];
    const kills = [playerKillsRef.current, teammateKillsRef.current["mate-0"] || 0, teammateKillsRef.current["mate-1"] || 0];
    let mvpIndex = 0;
    for (let i = 1; i < 3; i++) if (kills[i] > kills[mvpIndex]) mvpIndex = i;
    return (
      <div className="text-center">
        ...
        <p className="font-mono text-xs text-textDim mb-4">Final: {hud.roundsWon} — {hud.roundsLost}</p>
        <p className="font-pixel text-[10px] text-accentAmber mb-3">
          ⭐ MVP — {names[mvpIndex]} ({kills[mvpIndex]} kill{kills[mvpIndex] === 1 ? "" : "s"})
        </p>
        <div className="max-w-[360px] mx-auto rounded-lg border border-lineColor bg-bgPanel p-4 mb-5">
          <div className="flex justify-between font-mono text-[11px] text-textDim mb-2 pb-2 border-b border-lineColor">
            <span>You</span>
            <span>{playerKillsRef.current} kills · {playerHeadshotsRef.current} HS · {playerPlantsRef.current} plants · {playerDefusesRef.current} defuses</span>
          </div>
          {[0, 1].map((i) => (
            <div key={i} className="flex justify-between font-mono text-[11px] text-textDim py-1">
              <span>{teammateNamesRef.current[i] || `Teammate ${i + 1}`}</span>
              <span>{teammateKillsRef.current[`mate-${i}`] || 0} kills</span>
            </div>
          ))}
        </div>
        <button
          onClick={() => onFinish(finalScoreRef.current)}
          className="font-pixel text-[10px] px-6 py-3 rounded-md text-bgDeep"
          style={{ background: outcome === "victory" ? "#ffb703" : "#ff3ea5" }}
        >
          CONTINUE
        </button>
      </div>
