...
export default function OperationBlacksite({ onFinish, accentColor }) {
  ...
  function resetForRound() {
    ...
    // Bots used to spawn at the exact same fixed coordinate every
    // single round — the real cause of "predictable path each game,"
    // since even randomized wandering afterward starts from an
    // identical point every time. Same randomization approach
    // already used for the player's own spawn, applied per bot, each
    // one avoiding the others already placed so they don't stack.
    const usedSpawnPoints = [spawn];
    botsRef.current = map.botSpawns.map((s, i) => {
      const botSpawn = pickSafeSpawnPoint(s, map.walls, usedSpawnPoints, 90, 60);
      usedSpawnPoints.push(botSpawn);
      return { id: i, x: botSpawn.x, y: botSpawn.y, hp: Math.round(80 * DIFFICULTIES[difficultyRef.current].hpMult), state: "patrol", patrolTarget: { x: botSpawn.x + (Math.random() - 0.5) * 80, y: botSpawn.y + (Math.random() - 0.5) * 80 }, aim: 0, alertUntil: 0, lastShotAt: 0, alive: true };
    });
  ...
  function runRoundLoop() {
    simIntervalRef.current = setInterval(() => {
      ...
      if (p.alive) {
        ...
        // Auto-fire: an explicit design choice, not standard for a
        // tactical shooter (most deliberately don't do this, since
        // aim timing is normally part of the skill) — built exactly
        // as asked, with a genuinely tight tolerance so it only
        // triggers when a bot is precisely on the crosshair, not just
        // somewhere nearby, and a real wall check so it can't fire
        // through cover it couldn't otherwise see through.
        if (!firingRef.current) {
          for (const b of botsRef.current) {
            if (!b.alive) continue;
            const d = dist(p.x, p.y, b.x, b.y);
            if (d > BULLET_RANGE) continue;
            const angleToBot = Math.atan2(b.y - p.y, b.x - p.x);
            let angleDiff = Math.abs(angleToBot - p.aim);
            if (angleDiff > Math.PI) angleDiff = Math.PI * 2 - angleDiff;
            if (angleDiff > 0.04) continue;
            const blocked = segmentHitsWall(p.x, p.y, p.aim, d, mapRef.current.walls);
            if (blocked) continue;
            tryFire(now);
            break;
          }
  ...
  return (
    <div className="text-center relative" ref={wrapRef}>
      ...
      <div className="flex justify-between font-mono text-[11px] mt-2 max-w-[700px] mx-auto text-textDim">
        <span>👥 {hud.teammatesAlive}/2</span>
