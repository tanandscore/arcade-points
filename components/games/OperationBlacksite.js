...
function tryMoveBot(b, angle, speed, walls) {
...
export default function OperationBlacksite({ onFinish, accentColor }) {
  ...
  function resetForRound() {
    ...
    botsRef.current = map.botSpawns.map((s, i) => {
      const botSpawn = pickSafeSpawnPoint(s, map.walls, usedSpawnPoints, 130, 60);
  ...
  function botAI(b, now) {
    ...
    if (b.state === "plant") {
      ...
      if (d > 8) {
        ...
        const used = tryMoveBot(b, ang, BOT_SPEED, mapRef.current.walls);
    ...
    if (b.state === "retreat") {
      ...
      if (threat) {
        ...
        tryMoveBot(b, away, BOT_SPEED * 1.15, mapRef.current.walls);
    ...
    if (b.state === "engage" && b.engageTarget && b.engageTarget.obj.alive) {
      ...
      if (!segmentHitsWall(b.x, b.y, strafeAngle, strafeSpeed + 6, mapRef.current.walls)) {
        b.x += Math.cos(strafeAngle) * strafeSpeed;
        b.y += Math.sin(strafeAngle) * strafeSpeed;
      }
      ...
      if (td < 90) {
        tryMoveBot(b, b.aim + Math.PI, BOT_SPEED * 0.6, mapRef.current.walls);
      }
      ...
      const used = tryMoveBot(b, ang, BOT_SPEED * 0.7, mapRef.current.walls);
      if (used !== null) b.aim = used;
    } else {
      ...
      if (d < 8) {
        ...
        const used = tryMoveBot(b, ang, BOT_SPEED * 0.5, mapRef.current.walls);
        if (used !== null) b.aim = used;
      }
    }
