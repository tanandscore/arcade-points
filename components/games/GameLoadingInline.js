...
export default function GameLoadingInline() {
  useEffect(() => {
    // Fires on unmount — the moment this loading indicator is
    // replaced by the actual game component, which is exactly the
    // signal GameRunner listens for to measure real cold-launch time
    // (click to playable). A dispatched window event rather than a
    // prop, since next/dynamic's `loading` component can't receive
    // custom props from GameRunner.
    return () => {
      window.dispatchEvent(new CustomEvent("ap:game-loaded"));
    };
  }, []);
