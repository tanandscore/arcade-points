...
export default function NeonSnake({ onFinish, accentColor }) {
  ...
  useEffect(() => {
    if (!started) return;
    const tickMs = Math.max(65, BASE_TICK_MS - (levelRef.current - 1) * 9);
    intervalRef.current = setInterval(() => {
      setSnake((prev) => {
        ...
        if (ateFood) {
          ...
          const newLevel = Math.floor(foodCountRef.current / FOOD_PER_LEVEL) + 1;
      ...
      {!started ? (
        <div>
          <p className="mb-6 text-textDim">
            Use arrow keys (or the buttons below on mobile) to steer. Eat food, don't hit walls or yourself. Every
            {" "}{FOOD_PER_LEVEL} food is a new level and the snake speeds up.
          </p>
        ...
        <>
          ...
          <p className="font-mono text-xs mb-4 text-textDim">Length: <span className="text-textLight">{snake.length}</span> · Lvl {level}</p>
