...
export default function KingdomsOfAsh3D({ mapW, mapH, mapRef, buildingsRef, villagersRef, banditsRef, buildingColors, onWorldClick }) {
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(62, width / height, 0.1, 250);
    ...
    function road(fromX, fromZ, toX, toZ) {
      ...
      const tubeGeo = new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points), 24, 0.075, 6, false);
    ...
    // A real winding river, introduced per direct request — built the
    // same way as roads (a curve sampled through terrainHeight so it
    // sits correctly on the elevation), but wider, water-colored, and
    // flattened into a ribbon rather than a round tube, which is what
    // TubeGeometry produces by default and would look like a half-
    // buried pipe rather than a river. Positioned along the map's
    // right side with real winding curves, verified directly to clear
    // this map's Town Center and existing landmarks — a genuine,
    // stated limitation: the game's second map has a differently-
    // positioned Town Center, so this path is a reasonable placement
    // verified against one map's layout, not guaranteed clear on both.
    function river(waypoints, width) {
      const points = waypoints.map((p) => new THREE.Vector3(p.x, terrainHeight(p.x, p.z) - 0.06, p.z));
      const curve = new THREE.CatmullRomCurve3(points);
      const tubeGeo = new THREE.TubeGeometry(curve, 50, width, 8, false);
      const mesh = addMesh(tubeGeo, 0x2a6a8a, 0, 0, 0, { roughness: 0.15, metalness: 0.4 });
      mesh.scale.y = 0.12; // flattens the round tube into a real ribbon
    }
    river(
      [
        { x: MAP_W * 0.925, z: 0 },
        { x: MAP_W * 0.844, z: MAP_H * 0.15 },
        { x: MAP_W * 0.888, z: MAP_H * 0.29 },
        { x: MAP_W * 0.8, z: MAP_H * 0.43 },
        { x: MAP_W * 0.85, z: MAP_H * 0.58 },
        { x: MAP_W * 0.75, z: MAP_H * 0.73 },
        { x: MAP_W * 0.8, z: MAP_H * 0.875 },
        { x: MAP_W * 0.72, z: MAP_H },
      ],
      MAP_H * 0.025
    );
