...
export default function GrandPrixDuel({ onFinish, accentColor }) {
  ...
  const opponentRef = useRef({ t: 0.01, lateralOffset: 0, totalProgress: 0, speed: START_SPEED, seen: false });
  ...
  function startRaceVsAI() {
    ...
    opponentRef.current = { t: 0.01, lateralOffset: 0, totalProgress: 0, speed: START_SPEED, seen: false };
    sfx.select();
    setPhase("racing");
  }
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      if (isVsAIRef.current) {
        ...
        const curvatureAhead = trackCurvatureAt(curve, (bot.t || 0.01) + 0.015);
        const desiredSpeed = curvatureAhead > 1.0 ? MIN_SPEED * 1.6 : MAX_SPEED * 0.85;
        bot.speed = (bot.speed || START_SPEED) + (desiredSpeed - (bot.speed || START_SPEED)) * 0.04;
        const wobble = Math.sin(performance.now() / 850) * 0.35;
        bot.lateralOffset = Math.max(-0.6, Math.min(0.6, (bot.lateralOffset || 0) + wobble * 0.01));
        bot.t = (bot.t || 0.01) + bot.speed;
        bot.totalProgress = (bot.totalProgress || 0) + bot.speed;
        bot.seen = true;
      }
      ...
      const curvatureAheadMe = trackCurvatureAt(curve, s.t + 0.012);
    ...
    return (
      <div className="text-center">
        ...
        <div className="text-left max-w-xs mx-auto mb-6 space-y-2 text-sm text-textDim">
          ...
          <p>🌀 A real circuit with a chicane and a tight hairpin — brake before sharp turns or you'll lose grip.</p>
  ...
  return (
    <div className="text-center">
      ...
      {warnCorner && <p className="font-mono text-[10px] text-accentMagenta mb-1 ap-blink">⚠️ Sharp corner ahead — brake!</p>}
      {ahead !== null && (
