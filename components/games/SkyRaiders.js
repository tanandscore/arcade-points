...
const TICK_MS = 45;
const DIFFICULTY = {
  1: { label: "CADET", mult: 1, diveEvery: 70, speedMult: 0.9 },
  2: { label: "EASY", mult: 1.2, diveEvery: 60, speedMult: 1 },
  3: { label: "MEDIUM", mult: 1.45, diveEvery: 50, speedMult: 1.15 },
  4: { label: "HARD", mult: 1.75, diveEvery: 40, speedMult: 1.35 },
  5: { label: "ACE", mult: 2.1, diveEvery: 30, speedMult: 1.55 },
  6: { label: "LEGEND", mult: 2.5, diveEvery: 22, speedMult: 1.8 },
};
...
export default function SkyRaiders({ onFinish, accentColor }) {
  ...
  function begin(chosenLevel) {
    ...
    formationRef.current = { x: 15, y: 8, dir: 1, speed: 0.45 * diff.speedMult };
    setStarted(true);
    intervalRef.current = setInterval(() => {
      ...
      if (diveTimerRef.current > DIFFICULTY[levelRef.current].diveEvery) {
      ...
      setBullets((prevBullets) => {
        ...
        if (enemiesRef.current.every((e) => !e.alive) && diversRef.current.length === 0) {
          ...
          setEnemies(fresh);
          formationRef.current = { x: 15, y: 8, dir: 1, speed: (0.45 + waveRef.current * 0.12) * DIFFICULTY[levelRef.current].speedMult };
        }
