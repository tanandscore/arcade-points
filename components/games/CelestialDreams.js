...
const BIOMES = [
  ...
  { id: "peaks", name: "The Aurora Peaks", xStart: 1600, xEnd: 2400, skyTop: [8, 8, 26], skyBottom: [24, 18, 46], groundColor: [180, 190, 205], hasAurora: true, isNight: true, noFlora: true, creature: "moth" },
  { id: "starlitshore", name: "The Starlit Shore", xStart: 2400, xEnd: 3200, skyTop: [20, 24, 55], skyBottom: [60, 70, 110], groundColor: [90, 100, 130], hasWater: true, hasAurora: true, isNight: true, creature: "moth" },
];

const DIFFICULTIES = {
  wanderer: { label: "WANDERER", flightMax: 130, requiredFrac: 0.5, scoreMult: 1 },
  pilgrim: { label: "PILGRIM", flightMax: 100, requiredFrac: 0.6, scoreMult: 1.4 },
  seeker: { label: "SEEKER", flightMax: 75, requiredFrac: 0.85, scoreMult: 1.9 },
};
...
export default function CelestialDreams({ onFinish, accentColor }) {
  ...
  const [difficulty, setDifficulty] = useState("pilgrim");
  const [difficultyBests, setDifficultyBests] = useState({});
  const [hud, setHud] = useState({ restored: 0, totalBlooms: 16, flight: 100, biomeName: "" });
  ...
  const flightMaxRef = useRef(FLIGHT_MAX);
  const difficultyRef = useRef("pilgrim");
  ...
  useEffect(() => {
    fetch("/api/difficulty-scores?game=celestialdreams")
      .then((r) => r.json())
      .then((d) => setDifficultyBests(d.bests || {}))
      .catch(() => {});
  }, []);

  useEffect(() => () => stopAmbient(), []); // always clean up any playing ambient on unmount
  ...
  function resetRun() {
    ...
    flightMaxRef.current = DIFFICULTIES[difficultyRef.current].flightMax;
    flightRef.current = flightMaxRef.current;
  ...
  function endRun(won) {
    ...
    const score = Math.round((restoredCountRef.current * 40 + (won ? 400 : 0)) * diff.scoreMult);
    fetch("/api/difficulty-scores", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ game: "celestialdreams", difficulty: difficultyRef.current, score }),
    }).catch(() => {});
  ...
  function begin() {
    ...
    simIntervalRef.current = setInterval(() => {
      ...
      for (const s of stonesRef.current) {
        if (dist(p.x, p.y, s.x, s.y) < 26 && flightRef.current < flightMaxRef.current) {
          flightRef.current = flightMaxRef.current;
      ...
      setHud({
        ...
        flight: Math.round((flightRef.current / flightMaxRef.current) * 100),
      ...
      if (p.x > WORLD_W - 60 && restoredCountRef.current >= bloomsRef.current.length) {
        ...
        endRun(restoredCountRef.current >= bloomsRef.current.length * DIFFICULTIES[difficultyRef.current].requiredFrac);
      }
    }, TICK_MS);
  }
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      for (const r of reedsRef.current) {
        ...
        if (rBiome.noFlora) continue;
        const gy = groundYAt(r.x);
        const sway = Math.sin(r.sway) * 4;
        ctx.strokeStyle = rBiome.hasWater ? "#2e6a4a" : "#8a7040";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(rx, gy);
      ...
      for (const c of creaturesRef.current) {
        ...
        ctx.globalAlpha = 0.7;
        ctx.fillStyle = BIOMES[c.biome].isNight ? "#ffe9b8" : "#e8e2d6";
        ctx.beginPath();
    ...
    return (
      <div className="text-center max-w-md mx-auto">
        ...
        <p className="text-textDim text-sm mb-6">
          ...
          Ancient Bloom you stand near. There is no combat, no health, no threat — just four fading lands to walk
          through and return to color. Glowing pillars refill your flight.
        </p>
        <p className="font-mono text-[10px] text-textDim mb-2">CHOOSE HOW FAR YOU MUST WANDER</p>
