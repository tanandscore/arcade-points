...
export default function HorizonGuardian({ onFinish, accentColor }) {
  ...
  function begin() {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (spawnTimerRef.current > Math.max(35, 70 - levelRef.current * 5) && abductorsRef.current.length < Math.min(6, 4 + Math.floor(levelRef.current / 3))) {
      ...
      setBullets((prevBullets) => {
        ...
        if (rescuedId !== null) {
          ...
          rescuesRef.current += 1;
          if (rescuesRef.current >= RESCUES_PER_LEVEL) {
            rescuesRef.current = 0;
            levelRef.current += 1;
            setLevel(levelRef.current);
            scoreRef.current += 70 * levelRef.current;
            setScore(scoreRef.current);
            sfx.levelUp();
            haptics.success();
