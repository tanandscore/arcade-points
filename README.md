...
5. Before the first deploy, go to **Settings → Variables and Secrets** on this Worker, and add each of these as **Secrets** (values from Steps 2 & 3):
...
6. Deploy. You'll get a live URL like `https://arcade-points.yoursubdomain.workers.dev`.
7. Set `NEXT_PUBLIC_SITE_URL` to that exact URL in the same Variables screen, and redeploy so it picks up the change.
...
8. Go back to **Settings → Variables and Secrets** on the worker and update `NEXT_PUBLIC_SITE_URL` to `https://tapandscore.com`, then redeploy (**Deployments** tab → **Retry deployment** on the latest one, or push any small change to GitHub to trigger a fresh build).
...
3. In Cloudflare, replace `RAZORPAY_KEY_ID` and `RAZORPAY_KEY_SECRET` (Settings → Variables and Secrets) with the live values, then redeploy.
